...
from xtuner.v1.module.ultraep import UltraEPLayerRuntime, UltraEPManagerProvider
...
class _UltraEPWeightSyncForBackward(Function):
    ...
    A later virtual layer can overwrite them after this layer's forward.  This
    identity node sits immediately after expert compute, so its backward runs
    after combine backward but before the grouped-GEMM backward that needs the
    replica weights for DGrad.
    """
