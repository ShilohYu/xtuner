...
def test_ultra_ep_rejects_manual_microbatch_capacity():
    with pytest.raises(ValidationError, match="max_microbatches"):
        UltraEPConfig(num_redundant_experts_per_rank=1, max_microbatches=2)


def test_ultra_ep_requires_experts_evenly_sharded_by_ep():
    with pytest.raises(ValueError, match="n_routed_experts"):
        validate_ultraep_moe_compatibility(
            UltraEPConfig(num_redundant_experts_per_rank=1),
            ep_size=8,
            n_routed_experts=33,
            dispatcher="deepep",
...
def test_ultra_ep_moe_compatibility_rejects_disabled_ep():
...
def test_ultra_ep_accepts_supported_moe_compatibility():
    validate_ultraep_moe_compatibility(
        UltraEPConfig(num_redundant_experts_per_rank=1),
        ep_size=8,
        n_routed_experts=32,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"float8_enabled": True}, "BF16"),
        ({"moe_bias": True}, "expert bias"),
    ],
)
def test_ultra_ep_rejects_unsupported_moe_compatibility(overrides, match):
    kwargs = {
        "ep_size": 8,
        "n_routed_experts": 32,
        "dispatcher": "deepep",
...
def test_ultra_ep_rejects_mtp():
    ultraep = UltraEPConfig(num_redundant_experts_per_rank=1)

    with pytest.raises(ValueError, match="MTP expert layers"):
        validate_ultraep_mtp_compatibility(ultraep, mtp_enabled=True)

    validate_ultraep_mtp_compatibility(ultraep, mtp_enabled=False)
    validate_ultraep_mtp_compatibility(None, mtp_enabled=True)


def test_moe_decoder_layer_type_hints_are_runtime_resolvable():
    assert get_type_hints(MoEDecoderLayer.__init__)["ultraep_manager_provider"] == UltraEPManagerProvider | None
...
def test_ultra_ep_rejects_activation_recompute():
    ...
    with pytest.raises(ValueError, match="activation recompute"):
        validate_ultraep_fsdp_compatibility(ultraep, recompute_ratio=0.5)

    validate_ultraep_fsdp_compatibility(ultraep, recompute_ratio=0.0)


def test_ultra_ep_rejects_excess_intra_layer_microbatches():
    runtime = object.__new__(UltraEPLayerRuntime)
    runtime.max_microbatches = 1

    with pytest.raises(ValueError, match="max_microbatches=1"):
        runtime.validate_microbatch_capacity(2)


def test_ultra_ep_rejects_decoder_micro_batch_forward():
    layer = object.__new__(MoEDecoderLayer)
    layer._ultraep = object()
    hidden_states = torch.empty(1, 1, 2)

    with pytest.raises(RuntimeError, match="intra_layer_micro_batch == 1"):
        MoEDecoderLayer._micro_batch_forward(
            layer,
            hidden_states_list=[hidden_states, hidden_states],
            seq_ctx_list=[object(), object()],  # type: ignore[list-item]
            position_embeddings_list=[object(), object()],  # type: ignore[list-item]
        )
...
def test_ultra_ep_manager_provider_owns_model_layer_capacity():
    provider = UltraEPManagerProvider(
        group=FakeGroup(),  # type: ignore[arg-type]
        num_model_layers=20,
...
def test_ultra_ep_layer_runtime_configures_buffers_and_refreshes_weight_pointers():
    ...
    assert runtime._ensure_manager() is manager
